...
const FactChecking = () => {
    ...
    const handleAsk = async (question) => {
        setResponseTime(0)
        try {
            const response = await fetch("https://13.53.190.57/wiki-links-vector-store", {
                method: "POST",
                headers: {
    ...
    return (
        <main className="min-h-screen pt-12 w-[1000px] max-w-full px-4 mx-auto">
            <div className="flex justify-center py-4">
                <h1 className="text-[16px] font-bold">Wikipedia Vector Store Fact Checker</h1>
            </div>
            <div>
                <textarea
            ...
            {responseTime !== 0 && <div className="mt-5">Response Time: {responseTime}</div>}
