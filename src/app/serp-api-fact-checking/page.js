...
const FactChecking = () => {
  ...
  const handleAsk = async (question) => {
    setResponseTime(0);
    try {
      const response = await fetch("http://13.53.190.57/fact_check_serp", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        setResult(data[0].response);
        setResponseTime(data[0].responseTime);
      } else {
  ...
  return (
    <main className="min-h-screen pt-12 w-[1000px] max-w-full px-4 mx-auto">
      <div className="flex justify-center py-4">
        <h1 className="text-[16px] font-bold">Serp API Fact Checker</h1>
      </div>
      <div>
        <textarea
